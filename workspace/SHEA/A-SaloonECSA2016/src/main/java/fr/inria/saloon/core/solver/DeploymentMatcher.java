...
import featureModel.AttributeDeployment;
...
public class DeploymentMatcher {
	...
	static String NodeHAB= "<nodeFeature name=\"HAB\"> <<odeAttributes name=\"CPU\"><value xsi:type=\"type:IntValue\" value=\"1024\"/></NodeAttributes>"
			+ "<NodeCardinalities cardinalityMin=\"1\" cardinalityMax=\"1\"/> </nodeFeature>";
	static String NodeCloud="<nodeFeature name=\"Cloud\"><NodeAttributes name=\"CPUCloud\"><value xsi:type=\"type:IntValue\" value=\"1000\"/></NodeAttributes>"
			+ "<NodeCardinalities cardinalityMin=\"1\" cardinalityMax=\"1\"/></nodeFeature>";
	...
	public static void main(String[] args) throws IOException {
		...
		/**
		 * Load application and node model
		 */
...
//		IntVar node1 = deploymentBuilder.getCSPVariableFromFeature(nodeFeatures.get(0));
	...
	public static List<FMConstraint> retrieveConstraintFromFM(EObject featureModel) {		
		...
		while (fmContent.hasNext()) {
			...
			if (modelElement instanceof FMConstraint) {
				...
				System.out.println("from"+fromFeature.getName());
	...
	{ long startTime = System.currentTimeMillis();
	    ...
	    for (int fileContentLinesIndex = 0; fileContentLinesIndex < fileContentLines.length; fileContentLinesIndex++)
	    {
	        ...
	        fileContents.append(fileContentLines[fileContentLinesIndex] + System.getProperty("line.separator")//System.lineSeparator()
	        		);
	    }
	...
	public NodeFeature AddNodeFeature () {
		//create new Node Feature in the application model
		NodeFeature nodenew = null;
		nodenew.setName("entry.getValue().getName()");
		
		FeatCardinality value = null ;
		value.setCardinalityMin(1);
		value.setCardinalityMax(1);
		
		nodenew.setNodeCardinalities(value);
		
		HostedBy hostedby = null;
		hostedby.setTo(nodenew);
		nodenew.setHostedBy(hostedby);
		return nodenew;
